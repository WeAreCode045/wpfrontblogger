...
if ( ! class_exists( 'WPFRONTBLOGGER_AJAX' ) ) {
	...
	class WPFRONTBLOGGER_AJAX {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_wpfrontblogger_ai_select_products', array( $this, 'ai_select_products' ) );
		}
		...
		/**
		 * AI: Select WooCommerce products
		 */
		public function ai_select_products() {
			if ( ! wp_verify_nonce( $_POST['nonce'], 'wpfrontblogger_nonce' ) ) {
				wp_die( 'Security check failed' );
			}
			...
			if ( ! class_exists( 'WooCommerce' ) ) {
				wp_send_json_error( array( 'message' => __( 'WooCommerce is not active. Product selection requires WooCommerce.', 'wpfrontblogger' ) ) );
			}

			$content = wp_unslash( $_POST['content'] );
			if ( empty( $content ) ) {
				wp_send_json_error( array( 'message' => __( 'Content is required for product selection', 'wpfrontblogger' ) ) );
			}

			$result = wpfrontblogger_ai()->select_products( $content );
